...
import { foundItems } from '../mock/items.js'
...
export default function FoundItems() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 md:px-8 py-10">
      <div className="mb-8">
        <h1 className="font-display text-3xl text-cream">Found Items</h1>
        <p className="text-parchment mt-2">Handed in by students — claim yours with a quick verification.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-[220px_1fr] gap-8">
        ...
        <div>
          ...
          {loading ? (
            ...
            <EmptyState
              title="No found items match your search"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {filtered.map((item) => (
                <ItemCard key={item.id} item={item} type="found" className="w-full" />
              ))}
            </div>
          )}
        </div>
