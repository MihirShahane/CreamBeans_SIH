...
def report_lost_item(payload: ItemCreate, repo: Repository = Depends(repo_dependency)):
...
def report_found_item(payload: ItemCreate, repo: Repository = Depends(repo_dependency)):
...
def get_item(item_id: str, repo: Repository = Depends(repo_dependency)):
    return item_service.get_item_or_404(repo, item_id)


@router.get("/{item_id}/matches")
def get_matches(item_id: str, repo: Repository = Depends(repo_dependency)):
...
def update_item_status(
    item_id: str, payload: ItemStatusUpdate, repo: Repository = Depends(repo_dependency)
):
